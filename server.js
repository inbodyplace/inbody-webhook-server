...
app.post("/webhook", (req, res) => {
  ...
  if (payload.IsTempData === "true") {
    console.log(
      `[Webhook] Received temp-data event (UserID=${payload.UserID}). Skipping API fetch.`,
    );
  ...
  console.log(
    `[Webhook] Received — UserID=${payload.UserID}, Equip=${payload.Equip}, Time=${payload.TestDatetimes}`,
  );
  ...
  const event = storeEvent(
    payload,
    null,
    apiReady() ? "pending" : "skipped_no_config",
  );
...
async function fetchInBodyData(event) {
  ...
  try {
    const res = await axios.post(
      `${INBODY_API_BASE}/InBody/GetInBodyDataByID`,
      {
        UserID: UserID,
...
app.listen(PORT, () => {
  ...
  console.log(
    `  API ready : ${apiReady() ? "Yes" : "No (set INBODY_ACCOUNT and INBODY_API_KEY in .env)"}`,
  );
  console.log("────────────────────────────────────────────");
});
